...
void NArray::TestVisualisationFunctions(NArraySupport::StorageOption storage_option, std::fstream& output_file)
{
	...
	std::cout << "Visualisation Functions" << std::endl << std::endl;
	...
	std::cout << "Visualisation in 0D: Scalar (MakeValue)" << std::endl;
	...
	std::cout << "Visualisation in 1D: Vector (MakeRamped)" << std::endl;
	...
	std::cout << "Visualisation in 2D: Matrix (MakeTriangleValue)" << std::endl;
	...
	std::cout << "Visualisation in 3D: Array3D (MakeRandom)" << std::endl;
	...
	std::cout << "Visualisation in 3D: Array3D (MakeZero, MakeTriangleValue, ReColour)" << std::endl;
	...
	for (int i = 0; i < 2; i++)
	{
		...
		std::string use_transform_text = "false";
		if (i == 1)
		{
			use_transform = true;
			use_transform_text = "true";
		}
		std::cout << "Visualisation on the Complex Plane: Julia Set, use transform is " << use_transform_text << std::endl;
		stored_type_out1::Start(storage_option, image_width, image_height, use_transform, output_file, &multi_threading_mask);
		std::cout << "Visualisation on the Complex Plane: Mandelbrot Set, use transform is " << use_transform_text << std::endl;
